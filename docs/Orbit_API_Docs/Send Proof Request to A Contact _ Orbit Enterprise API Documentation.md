...
TheSend Proof Request to Contact APIenables the LOB to send a proof request to a contact, or peer. The API delivers the proof request to the contact using the protocol supported by the contact. The protocol of the recipient is known from the public DID document of the recipient.
...
POST /api/lob/{lob_id}/proof/send-contact HTTP/1.1
Host: 
