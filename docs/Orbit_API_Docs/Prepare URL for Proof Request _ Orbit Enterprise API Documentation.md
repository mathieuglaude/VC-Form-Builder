...
ThePrepare URL for Proof Request APIenables the LOB to receive a URL-based proof request, which can then be sent as an out-of-band proof request. This URL will be created based on the protocol specified. The client can determine how the URL is sent to the intended recipient of the proof request.

### The Prepare URL for Proof Request API is called to prepare a URL-based proof request

UUID received upon LOB registration

ID corresponding to the stored proof request for which a URL will be provided as a response
...
POST /api/lob/{lob_id}/proof/url?connectionless=true HTTP/1.1
Host: 
