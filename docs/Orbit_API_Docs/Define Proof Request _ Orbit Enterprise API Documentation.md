...
TheDefine Proof Request APIenables the LOB, or Line of Business, to create and store a proof request template. Upon successful creation, you will receive a proof request ID. The proof request can then be sent at a later time using this ID.

### The Define Proof Request API is called to create and store a proof request template
...
POST /api/lob/{lob_id}/define-proof-request HTTP/1.1
Host: 
...
{
  "success": true,
  "message": "ANONCREDS proof has been defined successfully.",
  "data": {
    "proofDefineId": 1
  }
}
